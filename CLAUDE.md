...
**Verifying an inheritance key's seed-phrase backup** ("Verify" on the card) runs `BackUpSeedPhraseActivity` (intro → verify-now/skip → instructions) and then re-adds the key from the restored device via `SignerIntroActivity`. Coldcard and air-gap mark the key verified inside their own add-key screens (`Mk4IntroFragment`, `AddAirgapSignerFragment` → `SetKeyVerifiedUseCase` → backup-success screen). Ledger instead runs `LedgerActivity` in `verifyXfpOnly` mode — it creates nothing, just checks the device reports `expectedXfp` and returns `EXTRA_VERIFIED_XFP` — and that result is relayed back up (`SignerIntroActivity` → `BackUpSeedPhraseActivity` → the key-list fragment's launcher), so `OnChainTimelockAddKeyListViewModel.setKeyVerified` marks the step. Both intermediate activities relay with `setResult` + `finish`; neither may go back to `startActivity` without breaking this.
