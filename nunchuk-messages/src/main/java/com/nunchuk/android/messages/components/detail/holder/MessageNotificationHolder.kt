...
internal class MessageNotificationHolder(val binding: ItemMessageNotificationBinding) : RecyclerView.ViewHolder(binding.root) {
    ...
    private fun bindRoomNameEvent(event: TimelineEvent, sender: String) {
        binding.notification.text = getString(R.string.nc_message_name_changed, sender, event.nameChange().orEmpty())
    }
    ...
    private fun bindRoomCreateEvent(event: TimelineEvent, sender: String) {
        ...
        binding.notification.text = getString(R.string.nc_message_create_room, content?.creator ?: sender)
    }
    ...
    private fun bindMembershipEvent(event: TimelineEvent, sender: String) {
        ...
        when (event.membership()) {
            INVITE -> binding.notification.text = getString(R.string.nc_message_member_invite_group, sender, user)
            JOIN -> binding.notification.text = getString(R.string.nc_message_member_join_group, user)
            KNOCK -> binding.notification.text = getString(R.string.nc_message_member_kick_group, user)
            LEAVE -> binding.notification.text = getString(R.string.nc_message_member_left_group, user)
            BAN -> getString(R.string.nc_message_member_ban_group, user)
            NONE -> {
            }
        }
    }
