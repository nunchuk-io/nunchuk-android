...
import com.nunchuk.android.main.databinding.ActivityAddSignerBinding
...
class AddSignerActivity : BaseActivity() {
    ...
    private fun openSignerInfo(signerName: String, signerSpec: String) {
        finish()
        navigator.openSignerInfoScreen(this, signerName, signerSpec)
    }

    private fun setupViews() {
        binding.addSignerViaQR.setOnClickListener { showToast("Scan QR coming soon") }
        binding.signerSpec.getEditTextView().height = resources.getDimensionPixelSize(R.dimen.nc_height_48)
