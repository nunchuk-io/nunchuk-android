...
class WalletAuthenticationViewModel @Inject constructor(
    ...
    private suspend fun loadPsbtForSignIn(psbt: String) {
        val result = getSignInDummyTxFromPsbt(
            GetSignInDummyTxFromPsbt.Param(psbt = psbt)
        )
        if (result.isSuccess) {
            _state.update {
                it.copy(
    ...
    private suspend fun resolveWallet(): Wallet? {
        ...
        val candidateIds = listOf(walletLocalId, args.walletId).filter { it.isNotBlank() }.distinct()
        candidateIds.forEach { id ->
            getWalletDetail2UseCase(id)
                .onFailure { Timber.tag(TAG).e(it, "Load wallet by id failed: $id") }
                .getOrNull()?.let { return it }
        }
        val descriptors = listOf(walletDescriptor, args.userData).filter { it.isNotBlank() }.distinct()
        descriptors.forEach { descriptor ->
            parseWalletDescriptorUseCase(descriptor)
                .onFailure { Timber.tag(TAG).e(it, "Parse wallet descriptor failed") }
                .getOrNull()?.let { return it }
        }
        Timber.tag(TAG).e(
            "No wallet for desktop key signing (localId=$walletLocalId, walletId=${args.walletId}, hasServerDescriptor=${walletDescriptor.isNotBlank()}, hasUserData=${args.userData.isNotBlank()})"
        )
        return null
    }
        ...
        viewModelScope.launch {
            ...
            parseTrezorSignTransactionResponseUseCase(
                ParseTrezorSignTransactionResponseUseCase.Param(
                    wallet = wallet,
                    psbt = dataToSign.value,
                    xfp = callback.xfp.ifBlank { signer.masterFingerprint },
    ...
    private suspend fun requestSignTransactionByTrezor(signerModel: SignerModel) {
        val wallet = resolveWallet()?.also { currentWallet = it }
            ?: run {
                _event.emit(WalletAuthenticationEvent.ShowError("Cannot load wallet for Trezor signing"))
                return
            }

        if (dataToSign.value.isBlank()) {
            _event.emit(WalletAuthenticationEvent.ShowError("Can not sign this transaction"))
            return
        }
        ...
        getTrezorSignTransactionDeeplinkUseCase(
            ...
            _event.emit(WalletAuthenticationEvent.ShowOpenTrezorSuiteConfirmation(deeplink))
        }.onFailure {
            _event.emit(WalletAuthenticationEvent.ShowError(it.message.orUnknownError()))
        }
    }
    ...
    private suspend fun requestSignTransactionByLedger(signerModel: SignerModel) {
        ...
        // wallet-less flows (change email, key recovery, invite members) describe no wallet at all,
        // so they keep going through the desktop app.
        val bsmsWallet = if (args.walletId.isBlank()) {
            val wallet = if (isSignInSignatureFlow) resolveWallet()?.also { currentWallet = it } else null
            wallet ?: run {
    ...
    private fun isTrezorSigner(singleSigner: SingleSigner): Boolean {
        return singleSigner.type == SignerType.HARDWARE && singleSigner.tags.contains(SignerTag.TREZOR)
    }
    ...
    private suspend fun uploadSignatureForSignIn(
        masterFingerprint: String,
        signature: String,
        signatures: MutableMap<String, String>,
    ): Boolean {
    ...
    private fun signIn(dummyTransactionUpdate: SignInDummyTransactionUpdate) {
        viewModelScope.launch {
            ...
            if (result.isSuccess && resultInit.isSuccess) {
                _event.emit(
                    WalletAuthenticationEvent.SignInSuccess(
    ...
    companion object {
        private const val TAG = "WalletAuthenticationViewModel"
