...
class ExportBBQRJSONUseCase @Inject constructor(
    @IoDispatcher dispatcher: CoroutineDispatcher,
    private val nativeSdk: NunchukNativeSdk
) : UseCase<ExportBBQRJSONUseCase.Param, List<String>>(dispatcher) {
    override suspend fun execute(parameters: Param): List<String> {
        return nativeSdk.exportBBQRJSON(
            value = parameters.value,
            minVersion = parameters.minVersion,
            maxVersion = parameters.maxVersion
        )
    }
